...
if __name__ == "__main__":
    ...
    # Define the transformation (THIS IS AN EXAMPLE)
    transform = transforms.Compose([
        transforms.Resize((800, 800)),  # ResNet expects 224x224 input
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),  # Standard ImageNet normalization
    ])

    # get the dataset
    dataset_path = os.path.join(os.getcwd(), '../Data/ImageNet/val') #WRONG DATASET

    dataset = datasets.ImageFolder(root=dataset_path, transform=transform)

    if profiler:
        profile_with(profiler, model, dataset, device, batch_size=batch_size, num_workers=workers, interval=0.1, compile = True, precision='fp16')
    else:
